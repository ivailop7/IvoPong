...
export default class MenuComponent extends React.Component<any, any> {
  ...
  cpuRules = "'Spacebar' to serve, 'Arrow keys' to move the paddle. \n\nOn mobile, tap upper/lower half of screen to move in that direction. The first player to win 10 points wins.";
  twoPlayerRules = "'Spacebar' to serve, 'Arrow keys' to move the right paddle. Keys 'W' and 'S' to move the left paddle. \n\nOn mobile, the player 1, can tap the top and bottom of left half of the screen to move, the player 2, the top and bottom of the right half to move. The first player to win 10 points wins.";
  ...
  render() {
    return this.state.cpuMode ? (
      ...
      <>
        {!this.state.readRules ? <this.rulesWindow><span>{this.twoPlayerRules}</span><this.styledButton onClick={() => this.setState({readRules: true})}>Got It</this.styledButton></this.rulesWindow> : <PongComponent twoPlayerMode={true} />}
      </>
    ) : (
      <>
        <div
